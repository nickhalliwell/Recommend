...
def predItemRatings(user_id, item_id, row, weightedAvgDict,predAspDict,typeDict, predItemsDict):
    '''
    ...
    typeDict: dict, type importance constant for each type
...
def RunModel(data, weightedAvgDict = {},predAspDict = {}, Train=True):
    '''
    ...
    (if Train == False only accuracy is returned)
    '''
    ...
    for row in data.itertuples():
        ...
        if not row.Index % 10000 and row.Index:
        ...
        predItemsDict = predItemRatings(user_id, item_id, row, weightedAvgDict,predAspDict,typeDict, predItemsDict)
...
train_ratings, compressedTest, itemAspectDict,lmbda, gamma,fullAspects = Loader.Netflix(10,5)
