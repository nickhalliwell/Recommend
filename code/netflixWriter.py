...
def Netflix(min_nr_movies_train, min_nr_movies_test, surprise=False, write=False):
	...
	if write:
		...
		out_path = 'imperial_ijcai/test_{}_{}/'.format(min_nr_movies_train, min_nr_movies_test)


		train_ratings.to_csv(os.path.join(out_path, 'train_ratings_{}_{}'.format(min_nr_movies_train, min_nr_movies_test), '.csv'))

		compressedTest.to_csv(os.path.join(out_path, 'compressedTest{}_{}'.format(min_nr_movies_train, min_nr_movies_test), '.csv'), index=False)


		with open(os.path.join(out_path, 'itemAspectDict_{}_{}'.format(min_nr_movies_train, min_nr_movies_test),'.json', 'w')) as IAD:

			json.dump(itemAspectDict, IAD)


		with open(os.path.join(out_path, 'lmbda_{}_{}'.format(min_nr_movies_train, min_nr_movies_test),'.json', 'w')) as LMBDA:

			json.dump(lmbda, LMBDA)


		with open(os.path.join(out_path, 'gamma_{}_{}'.format(min_nr_movies_train, min_nr_movies_test),'.json', 'w')) as GAMMA:

			json.dump(gamma, GAMMA)


		with open(os.path.join(out_path, 'fullAspects_{}_{}'.format(min_nr_movies_train, min_nr_movies_test),'.json', 'w')) as ASP:

			json.dump(fullAspects, ASP)


		with open(os.path.join(out_path, 'fullSims_{}_{}'.format(min_nr_movies_train, min_nr_movies_test),'.json', 'w')) as SIMS:

			json.dump(fullSims, SIMS)


		return

	return train_ratings, compressedTest, itemAspectDict,lmbda, gamma,fullAspects, fullSims


print(Netflix(20, 5, write=True))